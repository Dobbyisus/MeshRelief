...
public class PeerManager {
    ...
    /**
     * Updates a peer's signal strength.
     *
     * @param id the peer ID
     * @param signalStrength the new signal strength value
     * @throws IllegalArgumentException if id is null or empty
     * @throws PeerException if peer not found
     */
    public synchronized void updatePeerSignalStrength(String id, double signalStrength) throws PeerException {
        validateId(id);
        Peer peer = peers.get(id);
        if (peer == null) {
            throw new PeerException("Peer with ID '" + id + "' not found");
     ...
     *
     * @param id the peer ID
     * @param metadata the new metadata (can be null)
     * @throws IllegalArgumentException if id is null or empty
     * @throws PeerException if peer not found
     */
    public synchronized void updatePeerMetadata(String id, String metadata) throws PeerException {
        validateId(id);
        Peer peer = peers.get(id);
        if (peer == null) {
            throw new PeerException("Peer with ID '" + id + "' not found");
